...
MovingAverage<float> avrVpp(2, 0);
//int tempCounter = 0;
...
#define button 4
#define ledPin 2
#define difference 700  // Margin above baseline for pulse detection

const int edgeBuffSize = 25;
int vppBuffer[edgeBuffSize];
int currentIndex = 0, bufferIndex;

int upperTrigger = 2000;
int lowerTrigger = 1000;
bool ctDialogFlag = 0;

int c;

unsigned long lastPieceMillis = 0, lastBtnPressMillis = 0, autoSMV = 0, lastTrimMillis = 0;
...
bool hmiButtonFlag = 0;  /// should save to SD
int pieceCount = 0;
...
int trimCount = 0;          /////// should save to sd
int trainedTrimCount = 2;   /// set no of traning steps
...
void codeForTask1(void *parameter) {
  ...
  while (1) {
    ...
    // Supervisor LED blinking
    else if (S_mem == 1) {
      blinkLED(CRGB(255, 0, 255), CRGB(0, 0, 0), 1000);  // Magenta
    }

    // Mechanic LED blinking
    else if (M_mem == 1) {
      blinkLED(CRGB(0, 255, 255), CRGB(0, 0, 0), 1000);  // Cyan
    }
  }
...
void displayDWIN() {
  ...
  /**** shutdown function ****/
    ...
    if ((pageNo == "0") && (changeOP == 1)) {
      ...
      trainedTrimCount = rdline.toInt();
      if (ReadDone == 0) {
        trainedTrimCount = 1000;
      }
...
void ctThread() {
  if ((!autoCountMode && digitalRead(button) == 1) || (autoCountMode && hmiButtonFlag)) {
    ...
    } else if(buttonPressCount==2)lastBtnPressMillis=millis();

    if (!autoCountMode) {
      autoSMV = millis() - lastPieceMillis;
	lastPieceMillis = millis();
      if (autoSMV > (trainedSMV *0.6)) {
        pieceCount++;
        ...
        Serial.println("SMV mismatch (Received/Actual)");
        Serial.println(smv * 60000);
        Serial.println(autoSMV);
        dialogBox("**Warning**", "SMV Mismatch !", 3000);
      }
    ...
    if (trainingCountDown != 0 && buttonPressCount > 1) {
      ...
      if (trimCount > 0) {
        ...
        Serial.println("No trim detected !");  /// throw 0 trainned Area error
        dialogBox("**Warning**", "No CT Signal !", 3000);
      }
      ...
      if (trainingCountDown == 0) {
        ...
        trainedTrimCount = trimCount / trainingNum;
        waitTime = 1000;
        String trainnedA = String(trainedTrimCount);
        writeFile(SD, "/44.txt", trainnedA.c_str()); /*SD Save*/
        trainedSMV=lastTrimMillis-lastBtnPressMillis;
        String trainedSMV_M = String(trainedSMV);
        writeFile(SD, "/76.txt", trainedSMV_M.c_str()); /*SD Save*/  /// trainedSMV save to sd here              //// sd Save !!!!!!!!!!!!!!!

        trimCount = 0;  /// reset at last
        Serial.println("\nTrained Trims/SMV");
        Serial.println(trainedTrimCount);
        Serial.println(trainedSMV);
        delay(1900);
      }
  ...
  if (adcValue < 400 && !ctDialogFlag) {
    dialogBox("**Warning**", "CT Removed", 4800000);
    ctDialogFlag = 1;
  } else if (adcValue > 400) {
    pushToBuffer(adcValue);
    if (ctDialogFlag) dialogBox("**Warning**", "CT Connected", 1500);
    ctDialogFlag = 0;
  }
  ...
  // Serial.print(" ");
  // Serial.print(upperTrigger);
  // Serial.print(" ");
  // Serial.println(lowerTrigger);


  if (trimDetect()) {  /// triggers on a falling edge and caluculate the needledown time

    digitalWrite(ledPin, HIGH);
    trimCount++;
    unsigned long trimDelay = millis() - lastTrimMillis;
    if (minTrimTime > trimDelay) minTrimTime = trimDelay;
    lastTrimMillis = millis();
    Serial.println("Trim Count");
    Serial.println(trimCount);
    delay(900);
    populateBuffer();

    if (autoCountMode && trainingCountDown == 0 && trimCount >= trainedTrimCount) {
      trimCount = 0;
      autoSMV = millis() - lastPieceMillis;
	lastPieceMillis = millis();
      if (autoSMV > (trainedSMV * 0.6)){
      pieceCount++;      
      if (autoSMV < (trainedSMV * 120000)) avrAutoSMV.push(autoSMV);
      
      Serial.println("Piece Count/auto SMV");
      ...
      effi_cal();
      //delay(500);
      } else dialogBox("**Warning**", "SMV MISMATCH", 2000);
    }
...
bool trimDetect() {
  int maxValue = vppBuffer[0];
  int minValue = vppBuffer[0];

  // Find the min and max values in the buffer
  for (int i = 1; i < edgeBuffSize; i++) {
    if (vppBuffer[i] > maxValue) {
      maxValue = vppBuffer[i];
    }
    if (vppBuffer[i] < minValue) {
      minValue = vppBuffer[i];
    }
  }

  // Calculate peak-to-peak voltage
  int vpp = maxValue - minValue;

  // Detect pulse based on dynamic threshold
  if (vpp >= difference) {
    return true;  // Pulse detected
  } else {
    return false;  // No pulse detected
  }
}
